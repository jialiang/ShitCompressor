...
namespace Imazen.WebP.Extern {
    public static class LoadLibrary {
        ...
        public static bool EnsureLoadedByPath(string fullPath, bool throwException) {
            ...
            lock (lockObj) {
                if (loaded.TryGetValue(fullPath, out IntPtr handle)) {
                    return true;
                } else {
        ...
        public static bool AutoLoadNearby(string name, bool throwFailure) {
            ...
            return AutoLoad(name, new string[] { Path.GetDirectoryName(a.Location), Path.GetDirectoryName(new Uri(a.Location).LocalPath) }, throwFailure, throwFailure);
        }

        static readonly object lockObj = new object();
        static readonly Dictionary<string, IntPtr> loaded = new Dictionary<string, IntPtr>(StringComparer.OrdinalIgnoreCase);
